...
const tools = [
  ...
  },
  {
    ...
    input_schema: { type: 'object', properties: { query: { type: 'string', description: 'Nombre, apellido o DNI del paciente.' } }, required: ['query'] },
  },
  {
...
async function runTool(name: string, input: Record<string, unknown>, admin: ReturnType<typeof createClient>, professionalId: string): Promise<unknown> {
  ...
  if (name === 'consultar_historia_paciente') {
    ...
    return { count: matches.length, patients: matches.map((patient) => ({ id: patient.id, name: `${patient.apellido || ''}, ${patient.nombre || ''}`.trim(), dni: patient.dni, email: patient.email, obraSocial: patient.obraSocial, numeroAfiliado: patient.numeroAfiliado, plan: patient.plan, birthDate: patient.birthDate, diagnosticoPrincipal: patient.diagnosticoPrincipal, patologiasConocidas: patient.patologiasConocidas, patologiasCronicas: patient.patologiasCronicas, consultations: Array.isArray(patient.consultations) ? patient.consultations.slice(-10).map((consultation: Record<string, unknown>) => ({ date: consultation.date, motivoConsulta: consultation.motivoConsulta, diagnostico: consultation.diagnostico, detalleAtencion: consultation.detalleAtencion, pensamientoMedico: consultation.pensamientoMedico })) : [] })) }
  }
