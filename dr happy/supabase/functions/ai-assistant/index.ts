...
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !authorization) return jsonResponse(401, { success: false, message: 'Sesión profesional requerida.' })
  const admin = createClient(supabaseUrl, serviceRoleKey, { auth: { persistSession: false } })
  const { data: authData } = await admin.auth.getUser(authorization)
  if (!authData.user) return jsonResponse(401, { success: false, message: 'Sesión profesional inválida.' })
